      ...
      <div className="max-w-[1400px] mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
        ...
        <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
          ...
          <div className="flex items-center w-full gap-3 sm:w-auto">
             ...
             <button 
               onClick={() => openModal()}
               className="flex items-center justify-center gap-2 px-6 py-2.5 text-sm font-bold text-white bg-[#B91C1C] rounded-xl hover:bg-red-800 transition-all shadow-md shadow-red-200 active:scale-95 flex-1 sm:flex-none"
             >
          ...
          <div className="relative w-full max-w-lg bg-white rounded-[24px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
            ...
            <form className="p-6 space-y-5 overflow-y-auto md:p-8 custom-scrollbar" onSubmit={handleSaveProperty}>
              ...
              <div className="flex justify-end gap-3 pt-6 shrink-0">
                <button type="button" disabled={isSubmitting} onClick={closeModal} className="px-5 py-3 text-sm font-bold transition-colors text-slate-600 hover:bg-slate-100 rounded-xl disabled:opacity-50">Cancel</button>
                <button type="submit" disabled={isSubmitting} className="px-8 py-3 text-sm font-bold text-white bg-[#B91C1C] hover:bg-red-800 rounded-xl transition-all shadow-md active:scale-95 disabled:opacity-70 flex items-center gap-2">
...
const StatCard = ({ title, value, icon: Icon, trend, subtitle, theme, highlight, prefix = "" }) => {
  const themes = {
    ...
    red: 'text-red-500 bg-red-50' // Added Red theme for negative equity
  };
  ...
  return (
    <div className={cn(
      ...
      <div className="relative z-10">
        <h4 className="text-[32px] font-black tracking-tight font-mono">{prefix}{value}</h4>
        {(trend || subtitle) && (
  ...
  return (
    <div className="flex flex-col bg-white border border-slate-200 rounded-[24px] shadow-sm hover:shadow-lg transition-all group overflow-hidden">
       ...
       <div className="flex flex-col flex-1 p-6">
          ...
          {/* 🟢 UPGRADED: Underwater-aware Equity Progress Bar */}
...
const ZeroState = ({ onAdd }) => (
  <div className="flex flex-col items-center justify-center p-12 text-center bg-white border border-slate-200 shadow-sm rounded-[24px] mt-8 py-24">
    ...
    <button onClick={onAdd} className="flex items-center gap-2 px-8 py-3.5 bg-[#B91C1C] text-white font-bold rounded-xl hover:bg-red-800 transition-all shadow-md shadow-red-200 active:scale-95">
...
MyPropertiesPage.getLayout = (page) => (
