...
export default function LoanExplorer() {
  ...
  const [isMobileFiltersOpen, setIsMobileFiltersOpen] = useState(false); // UX Fix for mobile
  ...
  // --- DYNAMIC MATH ENGINE ---
  // Clamp values to prevent NaN or negative loans
  const safeHomeValue = Math.max(50000, params.homeValue);
  const safeDownPayment = Math.min(params.downPayment, safeHomeValue * 0.99); // Max 99% down
  ...
  const calculatePITI = useCallback((principal, annualRate, months, type) => {
    ...
    if (type === 'Government' || ltv > 80) {
      pmi = (principal * 0.006) / 12; // 0.6% avg PMI
    }
    ...
    try {
      ...
      setMarketRates([
        ...
        },
        {
          ...
          fees: 1500,
          popular: false,
          tags: ['Lower Credit Ok', 'Low Down Payment']
        },
        {
          id: 'arm-51',
          name: '5/1 ARM',
      ...
      setLoading(false);
      setIsUpdating(false);
      setIsMobileFiltersOpen(false); // Auto-close mobile drawer on successful fetch
    }
        ...
        >
           <div className="grid items-end grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-5">
              ...
              <button 
                type="submit" 
                disabled={isUpdating}
                className="h-12 w-full flex items-center justify-center gap-2 bg-[#0A1128] text-white font-bold text-sm rounded-xl hover:bg-slate-800 transition-all active:scale-95 disabled:opacity-70 shadow-lg shadow-slate-900/20"
              >
           ...
           {/* Mathematical Cross-Check & Warnings */}
  ...
  return (
    <div className={cn(
      ...
      <div className="flex flex-col flex-1 p-6">
        ...
        <div className="flex items-center gap-2 pb-6 mb-6 text-xs font-semibold border-b text-slate-500 border-slate-100">
           <span>{formatRate(data.apr)}% APR</span>
           <div className="relative group/tooltip">
             <Info size={14} className="transition-colors text-slate-400 cursor-help hover:text-slate-600" />
             <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-48 p-2 bg-slate-900 text-white text-[10px] rounded-lg opacity-0 invisible group-hover/tooltip:opacity-100 group-hover/tooltip:visible transition-all text-center z-10 shadow-xl pointer-events-none">
               Annual Percentage Rate includes interest and estimated fees.
             </div>
           </div>
        </div>

        {/* Feature Tags (Restored UX Fix) */}
        ...
        {/* 🟢 END-TO-END FIX: Passes fully populated query state to the application page */}
        <Link 
          href={{
...
RateCard.displayName = 'RateCard'; // Required for React.memo
